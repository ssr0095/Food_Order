...
const FoodDisplay = ({ category }) => {
  ...
  // Filter food list by category if not 'All'
  const filteredList =
    category === "All"
      ? foodList
      ...
      {filteredList.length > 0 ? (
        <div className="mt-8 grid grid-cols-foods items-center justify-center gap-8">
          {filteredList.map((item) => (
            <FoodCard
              key={item._id} // Use unique IDs as keys
              id={item._id}
              name={item.name}
        ...
        // <div
        //   role="status"
        //   className="my-28 flex w-full items-center justify-center"
        // >
        //   <svg
        //     aria-hidden="true"
        //     className="size-16 animate-spin fill-tomato text-gray-200 dark:text-gray-600"
        //     viewBox="0 0 100 101"
        //     fill="none"
